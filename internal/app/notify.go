...
// truthy value.
const noUpdateNotifierEnv = "PROMETHEUS_CLI_NO_UPDATE_NOTIFIER"

// updateCommandHint is the command the notice tells the user/agent to run to
// upgrade. There is no `prometheus-cli update` subcommand, so it points at the
// npm package the binary is distributed through.
const updateCommandHint = "npm install -g @angelmsger/prometheus-cli@latest"
